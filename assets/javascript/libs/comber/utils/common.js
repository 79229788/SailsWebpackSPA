...
export default {
  ...
  handleError: function (res) {
    ...
    if(res instanceof Error) {
      ...
      errorInfo.code = res.data.code || res.status;
      errorInfo.message = res.message || res.statusText;
      if(res.data) {
        errorInfo.message = res.data.message;
  ...
  getStorage: function(type) {
    if([
      Comber.KComberStorageType.localStorage,
      Comber.KComberStorageType.sessionStorage
    ].indexOf(type) > -1) {
      return window[type];
    }
