 ...
 * @param storageName
 */
Model.prototype.saveCurrentInStorage = function (type, storageName) {
  ...
  storage.setStorage(storageName, content, type);
};
/**
 * 删除当前本地的模型数据
 * @param type
 * @param storageName
 */
Model.deleteCurrentFromStorage = function (type, storageName) {
  const storage = new Comber.Storage();
 ...
 * @return {*|string}
 */
Model.getCurrentFromStorage = function (type, storageName) {
  const storage = new Comber.Storage();
