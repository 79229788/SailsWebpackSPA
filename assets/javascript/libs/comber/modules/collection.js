 ...
 * @param storageName
 */
Collection.prototype.saveCurrentInStorage = function (type, storageName) {
  ...
  storage.setStorage(storageName, this.toOrigin(), type);
};
/**
 * 删除当前本地的集合数据
 * @param type
 * @param storageName
 */
Collection.deleteCurrentFromStorage = function (type, storageName) {
  const storage = new Comber.Storage();
  storage.deleteStorage(storageName || this.storageName, type);
};
/**
 * 获取当前本地的集合数据
 * @param type
 ...
 * @return {*|string}
 */
Collection.getCurrentFromStorage = function (type, storageName) {
  const storage = new Comber.Storage();
  const json = storage.getStorage(storageName || this.storageName, type);
